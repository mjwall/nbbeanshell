 ...
 *
 * An instance of Interpreter can be used to source scripts and evaluate statements or expressions. <p>Here are some
 * examples:</p>
 *
 * <p><blockquote><pre>
 ...
 * </pre></blockquote></p>
 *
 * <p>In the above examples we showed a single interpreter instance, however you may wish to use many instances,
 * depending on the application and how you structure your scripts. Interpreter instances are very light weight to
 * create, however if you are going to execute the same script repeatedly and require maximum performance you should
 * consider scripting the code as a method and invoking the scripted method each time on the same interpreter instance
 * (using eval()).</p>
 *
 * <p>See the BeanShell User's Manual for more information.</p>
 */
public class Interpreter implements Runnable, ConsoleInterface, Serializable {
    ...
    /*
     * Debug utils are static so that they are reachable by code that doesn't necessarily have an interpreter reference
     * (e.g. tracing in utils). In the future we may want to allow debug/trace to be turned on on a per interpreter
     * basis, in which case we'll need to use the parent reference in some way to determine the scope of the command
     ...
     *
     * <p>Note: This is here for completeness. If you're using this a lot it may be an indication that you are doing
     * more work than you have to. For example, caching the interpreter instance rather than the namespace should not
     * add a significant overhead. No state other than the debug status is stored in the interpreter.</p>
     *
     * <p>All features of the namespace can also be accessed using the interpreter via eval() and the script variable
     * 'this.namespace' (or global.namespace as necessary).</p>
     */
    ...
    public void run() {
        ...
        /*
         * We'll print our banner using eval(String) in order to exercise the parser and get the basic expression
         * classes loaded... This ameliorates the delay after typing the first statement.
         */
        if (interactive) {
            try {
                eval("printBanner();");
        ...
        while (!eof) {
            try {
                ...
                /*
                 * We get stuck in infinite loops here when unicode escapes fail. Must re-init the char stream reader
    ...
    /*
     * Can't this be combined with run()? run seems to have stuff in it for interactive vs. non-interactive... compare
     * them side by side and see what they do differently, aside from the exception handling.
     */
    ...
    // end source and eval
    /**
     * Print an error message in a standard format on the output stream associated with this interpreter. On the GUI
     * console this will appear in red, etc.
     */
    @Override
    ...
    // ConsoleInterface
    // The interpreter reflexively implements the console interface that it uses. Should clean this up by using an inner
    // class to implement the console for us.
    /**
    ...
    public Object get(String name) throws EvalError {
        try {
            return Primitive.unwrap(globalNameSpace.get(name, this));
    ...
    /**
     * <p>Get a reference to the interpreter (global namespace), cast to the specified interface type. Assuming the
     * appropriate methods of the interface are defined in the interpreter, then you may use this interface from Java,
     * just like any other Java object.</p>
     *
     * <p>For example:
     * <pre>
     * Interpreter interpreter = new Interpreter();
     ...
     * </pre></p>
     *
     * <p>Note that the interpreter does *not* require that any or all of the methods of the interface be defined at the
     * time the interface is generated. However if you attempt to invoke one that is not defined you will get a runtime
     * exception.</p>
     *
     * <p>Note also that this convenience method has exactly the same effect as evaluating the script:
     * <pre>
     * (Type)this;
     * </pre></p>
     *
     * <p>For example, the following is identical to the previous example:</p>
     *
     * <p><pre>
     * // Fetch a reference to the interpreter as a Runnable
     ...
     * </pre></p>
     *
     * <p><em>Version requirement</em> Although standard Java interface types are always available, to be used with
     * arbitrary interfaces this feature requires that you are using Java 1.3 or greater.</p>
     *
     * @throws EvalError if the interface cannot be generated because the version of Java does not support the proxy
     * mechanism.
     */
     ...
     */
    protected JJTParserState get_jjtree() {
        return parser.jjtree;
    }

    protected boolean Line() throws ParseException {
        return parser.Line();
    }
    ...
    /**
     * <p>Set an external class loader to be used as the base classloader for BeanShell. The base classloader is used
     * for all classloading unless/until the addClasspath()/setClasspath()/reloadClasses() commands are called to modify
     * the interpreter's classpath. At that time the new paths /updated paths are added on top of the base classloader.
     * </p>
     *
     * <p>BeanShell will use this at the same point it would otherwise use the plain Class.forName(). I.e. if no
     * explicit classpath management is done from the script (addClassPath(), setClassPath(), reloadClasses()) then
     * BeanShell will only use the supplied classloader. If additional classpath management is done then BeanShell will
     * perform that in addition to the supplied external classloader. However BeanShell is not currently able to reload
     * classes supplied through the external classloader.</p>
     *
    ...
    /**
     * Get the class manager associated with this interpreter (the BshClassManager of this interpreter's global
     * namespace). This is primarily a convenience method.
     */
    ...
    /**
     * <p>Set strict Java mode on or off. This mode attempts to make BeanShell syntax behave as Java syntax, eliminating
     * conveniences like loose variables, etc. When enabled, variables are required to be declared or initialized before
     * use and method arguments are reqired to have types.</p>
     *
     * <p>This mode will become more strict in a future release when classes are interpreted and there is an alternative
     * to scripting objects as method closures.</p>
     */
    ...
    static void staticInit() {
        /*
         * Apparently in some environments you can't catch the security exception at all... e.g. as an applet in IE ...
         * will probably have to work around
         */
        try {
    ...
    /**
     * Specify the source of the text from which this interpreter is reading. Note: there is a difference between what
     * file the interrpeter is sourcing and from what file a method was originally parsed. One file may call a method
     * sourced from another file. See SimpleNode for origination file info.
     *
     * @see bsh.SimpleNode#getSourceFile()
     */
    public String getSourceFileInfo() {
        return (sourceFileInfo != null) ? sourceFileInfo : "<unknown source>";
    }

    /**
     * <p>Get the parent Interpreter of this interpreter, if any. Currently this relationship implies the following:</p>
     * <ol> <li>Parent and child share a BshClassManager</li> <li>Children indicate the parent's source file information
     * in error reporting.</li> </ol> <p>When created as part of a source() / eval() the child also shares the parent's
     * namespace. But that is not necessary in general.</p>
     */
    ...
    /**
     * Get the prompt string defined by the getBshPrompt() method in the global namespace. This may be from the
     * getBshPrompt() command or may be defined by the user as with any other method. Defaults to "bsh % " if the method
    ...
    /**
     * <p>Specify whether, in interactive mode, the interpreter exits Java upon end of input. If true, when in
     * interactive mode the interpreter will issue a System.exit(0) upon eof. If false the interpreter no System.exit()
     * will be done. </p>
     *
     * <p>Note: if you wish to cause an EOF externally you can try closing the input stream. This is not guaranteed to
     * work in older versions of Java due to Java limitations, but should work in newer JDK/JREs. (That was the
     * motivation for the Java NIO package).</p>
     */
    ...
    /**
     * Turn on/off the verbose printing of results as for the show() command. If this interpreter has a parent the call
     * is delegated. See the BeanShell show() command.
     */
    ...
    /**
     * Show on/off verbose printing status for the show() command. See the BeanShell show() command. If this interpreter
     * has a parent the call is delegated.
     */
