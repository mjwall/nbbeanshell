...
/**
 * A namespace	in which methods, variables, and imports (class names) live. This is package public because it is used in
 * the implementation of some bsh commands. However for normal use you should be using methods on bsh.Interpreter to
 * interact with your scripts. <p>
 *
 * A bsh.This object is a thin layer over a NameSpace that associates it with an Interpreter instance. Together they
 * comprise a Bsh scripted object context. <p>
 *
 * Note: I'd really like to use collections here, but we have to keep this compatible with JDK1.1
 */
/*
 * Thanks to Slava Pestov (of jEdit fame) for import caching enhancements. Note: This class has gotten too big. It
 * should be broken down a bit.
 */
public class NameSpace
    ...
    /**
     * The name of this namespace. If the namespace is a method body namespace then this is the name of the method. If
     * it's a class or class instance then it's the name of the class.
     */
    ...
    boolean isMethod;
    /**
     * Note that the namespace is a class body or class instance namespace. This is used for controlling static/object
     * import precedence, etc.
     */
    /*
     * Note: We can move this class related behavior out to a subclass of NameSpace, but we'll start here.
     */
    boolean isClass;
    Class classStatic;
     ...
     * Only unqualified class names are cached here (those which might be imported). Qualified names are always absolute
     * and are cached by BshClassManager.
     */
    ...
    // Begin constructors
    /**
     * @parent the parent namespace of this namespace. Child namespaces inherit all variables and methods of their
     * parent and can (of course) override / shadow them.
     */
    ...
    /**
     * Set the node associated with the creation of this namespace. This is used in debugging and to support the
     * getInvocationLine() and getInvocationText() methods.
     */
    ...
    /**
     * Set the variable through this namespace. This method obeys the LOCALSCOPING property to determine how variables
     * are set. <p> Note: this method is primarily intended for use internally. If you use this method outside of the
     * bsh package and wish to set variables with primitive values you will have to wrap them using bsh.Primitive.
     *
     * @see bsh.Primitive <p> Setting a new variable (which didn't exist before) or removing a variable causes a
     * namespace change.
     *
    ...
    /**
     * Set the value of a the variable 'name' through this namespace. The variable may be an existing or non-existing
     * variable. It may live in this namespace or in a parent namespace if recurse is true. <p> Note: This method is not
     * public and does *not* know about LOCALSCOPING. Its caller methods must set recurse intelligently in all
     * situations (perhaps based on LOCALSCOPING).
     *
     * <p> Note: this method is primarily intended for use internally. If you use this method outside of the bsh package
     * and wish to set variables with primitive values you will have to wrap them using bsh.Primitive.
     *
     * @see bsh.Primitive <p> Setting a new variable (which didn't exist before) or removing a variable causes a
     * namespace change.
     *
     * @param strictJava specifies whether strict java rules are applied.
     * @param recurse determines whether we will search for the variable in our parent's scope before assigning locally.
     */
    void setVariable(
        ...
        } else // No previous variable definition found here (or above if recurse)
        {
            ...
            varScope.variables.put(
                    name, createVariable(name, value, null/*
                     * modifiers
                     */));
    ...
    /**
     * Get the methods defined in this namespace. (This does not show methods in parent namespaces). Note: This will
     * probably be renamed getDeclaredMethods()
     */
    ...
    /**
     * Get the parent namespace. Note: this isn't quite the same as getSuper(). getSuper() returns 'this' if we are at
     * the root namespace.
     */
    ...
    /**
     * Get the top level namespace or this namespace if we are the top. Note: this method should probably return type
     * bsh.This to be consistent with getThis();
     */
    ...
    /**
     * A This object is a thin layer over a namespace, comprising a bsh object context. It handles things like the
     * interface types the bsh object supports and aspects of method invocation on it. <p>
     *
     * The declaringInterpreter is here to support callbacks from Java through generated proxies. The scripted object
     * "remembers" who created it for things like printing messages and other per-interpreter phenomenon when called
     * externally from Java.
     */
    /*
     * Note: we need a singleton here so that things like 'this == this' work (and probably a good idea for speed).
     *
     * Caching a single instance here seems technically incorrect, considering the declaringInterpreter could be
     * different under some circumstances. (Case: a child interpreter running a source() / eval() command ). However the
     * effect is just that the main interpreter that executes your script should be the one involved in call-backs from
     * Java.
     *
     * I do not know if there are corner cases where a child interpreter would be the first to use a This reference in a
     * namespace or if that would even cause any problems if it did... We could do some experiments to find out... and
     * if necessary we could cache on a per interpreter basis if we had weak references... We might also look at
     * skipping over child interpreters and going to the parent for the declaring interpreter, so we'd be sure to get
     * the top interpreter.
     */
...
//System.out.println("experiment: creating class manager");
    ...
    /**
     * Get the specified variable in this namespace or a parent namespace. <p> Note: this method is primarily intended
     * for use internally. If you use this method outside of the bsh package you will have to use Primitive.unwrap() to
     ...
     *
     * @param recurse If recurse is true then we recursively search through parent namespaces for the variable. <p>
     * Note: this method is primarily intended for use internally. If you use this method outside of the bsh package you
     * will have to use Primitive.unwrap() to get primitive values.
     * @see Primitive#unwrap( Object )
     *
     * @return The variable value or Primitive.VOID if it is not defined.
     */
    ...
    /**
     * Declare a variable in the local scope and set its initial value. Value may be null to indicate that we would like
     * the default value for the variable type. (e.g. 0 for integer types, null for object types). An existing typed
     * variable may only be set to the same type. If an untyped variable of the same name exists it will be overridden
     * with the new typed var. The set will perform a Types.getAssignableForm() on the value if necessary.
     *
     * <p> Note: this method is primarily intended for use internally. If you use this method outside of the bsh package
     * and wish to set variables with primitive values you will have to wrap them using bsh.Primitive.
     *
     ...
     *
     * @param name is here just to allow the error message to use it protected void checkVariableModifiers( String name,
     * Modifiers modifiers ) throws UtilEvalError { if ( modifiers!=null && modifiers.hasModifier("static") ) throw new
     * UtilEvalError( "Can't declare static variable outside of class: "+name ); }
     */
    /**
    ...
    /**
     * Get the bsh method matching the specified signature declared in this name space or a parent. <p> Note: this
     * method is primarily intended for use internally. If you use this method outside of the bsh package you will have
     * to be familiar with BeanShell's use of the Primitive wrapper class.
     *
     * @see bsh.Primitive
     * @return the BshMethod or null if not found
     * @param declaredOnly if true then only methods declared directly in this namespace will be found and no inherited
     * or imported methods will be visible.
     */
    ...
    /**
     * Import scripted or compiled BeanShell commands in the following package in the classpath. You may use either "/"
     * path or "." package notation. e.g. importCommands("/bsh/commands") or importCommands("bsh.commands") are
     * equivalent. If a relative path style specifier is used then it is made into an absolute path by prepending "/".
     */
    ...
    /**
     * A command is a scripted method or compiled command class implementing a specified method signature. Commands are
     * loaded from the classpath and may be imported using the importCommands() method.
     * <p/>
     *
		This method searches the imported commands packages for a script or command object corresponding to the name of
     * the method. If it is a script the script is sourced into this namespace and the BshMethod for the requested
     * signature is returned. If it is a compiled class the class is returned. (Compiled command classes implement
     * static invoke() methods).
     * <p/>
     *
		The imported packages are searched in reverse order, so that later imports take priority. Currently only the
     * first object (script or class) with the appropriate name is checked. If another, overloaded form, is located in
     * another package it will not currently be found. This could be fixed.
     * <p/>
     *
     ...
     *
     * @throws UtilEvalError on error in parsing the script or if the the method is not found after parsing the script.
     */
    /*
    ...
    private BshMethod loadScriptedCommand(
        ...
        try {
            ...
            /*
             * Here we catch any EvalError from the interpreter because we are using it as a tool to load the command,
             * not as part of the execution path.
             */
        ...
        /*
         * if ( meth == null ) throw new UtilEvalError("Loaded resource: " + resourcePath + "had an error or did not
         * contain the correct method" );
         */
    ...
    /**
     * Load a class through this namespace taking into account imports. The class search will proceed through the parent
     ...
     * Load a class through this namespace taking into account imports. <p>
     *
     * Check the cache first. If an unqualified name look for imported class or package. Else try to load absolute name.
     * <p>
     *
     * This method implements caching of unqualified names (normally imports). Qualified names are cached by the
     * BshClassManager. Unqualified absolute class names (e.g. unpackaged Foo) are cached too so that we don't go
    ...
    /**
     * Try to make the name into an imported class. This method takes into account only imports (class or package) found
     * directly in this NameSpace (no parent chain).
     */
    private Class getImportedClassImpl(String name)
        ...
        /*
         * Try imported packages, e.g. "import foo.bar.*;" in reverse order of import... (give later imports
         * precedence...)
         */
        if (importedPackages != null) {
        ...
        /*
         * Try super import if available Note: we do this last to allow explicitly imported classes and packages to take
         * priority. This method will also throw an error indicating ambiguity if it exists...
         */
    ...
    /**
     * Invoke a method in this namespace with the specified args and interpreter reference. No caller information or
     * call stack is required. The method will appear as if called externally from Java. <p>
     *
     * @see bsh.This.invokeMethod( String methodName, Object [] args, Interpreter interpreter, CallStack callstack,
     * SimpleNode callerInfo, boolean )
     */
    ...
    public void loadDefaultImports() {
        /**
         * Note: the resolver looks through these in reverse order, per precedence rules... so for max efficiency put
         * the most common ones later.
         */
        importClass("bsh.EvalError");
        importClass("bsh.Interpreter");
        importPackage("javax.swing.event");
    ...
    /**
     * This is the factory for Name objects which resolve names within this namespace (e.g. toObject(), toClass(),
     * toLHS()). <p>
     *
     * This was intended to support name resolver caching, allowing Name objects to cache info about the resolution of
     * names for performance reasons. However this not proven useful yet. <p>
     *
     * We'll leave the caching as it will at least minimize Name object creation. <p>
     *
     * (This method would be called getName() if it weren't already used for the simple name of the NameSpace) <p>
     *
    ...
    /**
     * This is a helper method for working inside of bsh scripts and commands. In that context it is impossible to see a
     * ClassIdentifier object for what it is. Attempting to access a method on a ClassIdentifier will look like a static
     * method invocation.      *
    ...
    /**
     * Clear all variables, methods, and imports from this namespace. If this namespace is the root, it will be reset to
     * the default imports.
     *
    ...
    /**
     * Import a compiled Java object's methods and variables into this namespace. When no scripted method / command or
     * variable is found locally in this namespace method / fields of the object will be checked. Objects are checked in
     * the order of import with later imports taking precedence.
     * <p/>
     */
    /*
