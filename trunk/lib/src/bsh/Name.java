...
/**
 * What's in a name? I'll tell you... Name() is a somewhat ambiguous thing in the grammar and so is this. <p>
 *
 * This class is a name resolver. It holds a possibly ambiguous dot separated name and reference to a namespace in which
 * it allegedly lives. It provides methods that attempt to resolve the name to various types of entities: e.g. an
 * Object, a Class, a declared scripted BeanShell method. <p>
 *
 * Name objects are created by the factory method NameSpace getNameResolver(), which caches them subject to a class
 * namespace change. This means that we can cache information about various types of resolution here. Currently very
 * little if any information is cached. However with a future "optimize" setting that defeats certain dynamic behavior
 * we might be able to cache quite a bit.
 */
/*
 * <strong>Implementation notes</strong> <pre> Thread safety: all of the work methods in this class must be synchronized
 * because they share the internal intermediate evaluation state.
 *
 * Note about invokeMethod(): We could simply use resolveMethod and return the MethodInvoker (BshMethod or JavaMethod)
 * however there is no easy way for the AST (BSHMehodInvocation) to use this as it doesn't have type information about
 * the target to resolve overloaded methods. (In Java, overloaded methods are resolved at compile time... here they are,
 * of necessity, dynamic). So it would have to do what we do here and cache by signature. We now do that for the client
 * in Reflect.java.
 *
 ...
 *
 * // does *not* work for( caller=this.caller; caller != null; caller = caller.caller );
 *
 * is prohibited by the restriction that you can only call .caller on a literal	this or caller reference. The effect is
 * that magic caller reference only works through the current 'this' reference. The real explanation is that This
 * referernces do not really know anything about their depth on the call stack. It might even be hard to define such a
 * thing...
 *
...
class Name implements java.io.Serializable {
    ...
    /**
     * The last part of the name evaluated. This is really only used for this, caller, and super resolution.
     */
     ...
     *
     * Throws EvalError on various failures. A null object value is indicated by a Primitive.NULL. A return type of
     * Primitive.VOID comes from attempting to access an undefined variable.
     *
     * Some cases: myVariable myVariable.foo myVariable.foo.bar java.awt.GridBagConstraints.BOTH
     * my.package.stuff.MyClass.someField.someField...
     *
     * Interpreter reference is necessary to allow resolution of "this.interpreter" magic field. CallStack reference is
     * necessary to allow resolution of "this.caller" magic field. "this.callstack" magic field.
     */
    ...
    /**
     * @see toObject()
     * @param forceClass if true then resolution will only produce a class. This is necessary to disambiguate in cases
     * where the grammar knows that we want a class; where in general the var path may be taken.
     */
    ...
    /**
     * Get the next object by consuming one or more components of evalName. Often this consumes just one component, but
     * if the name is a classname it will consume all of the components necessary to make the class identifier.
     */
    private Object consumeNextObjectField(
            ...
            throws UtilEvalError {
        /*
         * Is it a simple variable name? Doing this first gives the correct Java precedence for vars vs. imported class
         * names (at least in the simple case - see tests/precedence1.bsh). It should also speed things up a bit.
         */
        ...
        /*
         * Is it a bsh script variable reference? If we're just starting the eval of name (no base object) or we're
         * evaluating relative to a This type reference check.
         */
        ...
        /*
         * If we didn't find a class or variable name (or prefix) above there are two possibilities:
         *
         * - If we are a simple name then we can pass as a void variable reference. - If we are compound then we must
         * fail at this point.
         */
        if (evalBaseObject == null) {
        ...
        /*
         * -------------------------------------------------------- After this point we're definitely evaluating
         * relative to a base object. --------------------------------------------------------
         */
     ...
     *
     * This is the general variable resolution method, accomodating special fields from the This context. Together the
     * namespace and interpreter comprise the This context. The callstack, if available allows for the this.caller
     * construct. Optionally interpret special "magic" field names: e.g. interpreter.
     * <p/>
     *
		@param callstack may be null, but this is only legitimate in special cases where we are sure resolution will not
     * involve this.caller.
     *
    ...
    Object resolveThisFieldReference(
        ...
        if (varName.equals("this")) {
            /*
             * Somewhat of a hack. If the special fields are visible (we're operating relative to a 'this' type already)
             * dissallow further .this references to prevent user from skipping to things like super.this.caller
             */
        ...
        /*
         * Some duplication for "super". See notes for "this" above If we're in an enclsing class instance and have a
         * superclass instance our super is the superclass instance.
         */
     ...
     * @throws ClassPathException (type of EvalError) on special case of ambiguous unqualified name after super import.
     */
    ...
    synchronized public LHS toLHS(
        ...
        if (obj instanceof This) {
            ...
            /*
             * If this was a literal "super" reference then we allow recursion in setting the variable to get the normal
             * effect of finding the nearest definition starting at the super scope. On any other resolution qualified
             * by a 'this' type reference we want to set the variable directly in that scope. e.g. this.x=5; or
             * someThisType.x=5;
             *
    ...
    /**
     * Invoke the method identified by this name. Performs caching of method resolution using SignatureKey. <p>
     *
     * Name contains a wholely unqualfied messy name; resolve it to ( object | static prefix ) + method name and invoke.
     * <p>
     *
     * The interpreter is necessary to support 'this.interpreter' references in the called code. (e.g. debug()); <p>
     *
     * <pre>
     * Some cases:
     *
    ...
    /**
     * Invoke a locally declared method or a bsh command. If the method is not already declared in the namespace then
     * try to load it as a resource from the imported command path (e.g. /bsh/commands)
     */
    /*
     * Note: the bsh command code should probably not be here... we need to scope it by the namespace that imported the
     * command... so it probably needs to be integrated into NameSpace.
     */
    ...
    /*
     * private String getHelp( String name ) throws UtilEvalError { try { // should check for null namespace here return
     * get( "bsh.help."+name, null/interpreter/ ); } catch ( Exception e ) { return "usage: "+name; } }
     *
     * private String getHelp( Class commandClass ) throws UtilEvalError { try { return
     * (String)Reflect.invokeStaticMethod( null/bcm/, commandClass, "usage", null ); } catch( Exception e ) return
     * "usage: "+name; } }
     */
