...
/**
 *
 * This file is derived from Pat Niemeyer's free utilities package. Now part of BeanShell.
 *
 ...
 *
 * Two static util	methods	are provided to	help read the text of a	template from a stream (perhaps a URL or	resource).
 * e.g.
 *
 * @author	Pat Niemeyer
 */
public class SimpleTemplate {
    ...
    /**
     * Get a template by name, with caching. Note: this should be updated to search the resource path first, etc.
     *
     * Create a new instance of a template from the specified file.
     *
     * The file text is cached	so lookup is fast. Failure to find the file is	also cached so the read	will not happen
     * twice.
     *
     */
    public static SimpleTemplate getTemplate(String file) {
        ...
        } else // Quick check prevents	trying each time
    ...
    /**
     * Find the starting (inclusive) and ending (exclusive) index of the named template and return them as a two	element
     * int []. Or return null if the param is not found.
     */
