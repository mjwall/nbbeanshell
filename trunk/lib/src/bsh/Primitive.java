...
/**
 * Wrapper for primitive types in Bsh. This is package public because it is used in the implementation of some bsh
 * commands.
 *
 * See the note in LHS.java about wrapping objects.
 */
/*
 * Note: this class is final because we may test == Primitive.class in places. If we need to change that search for
 * those tests.
 */
public final class Primitive implements ParserConstants, java.io.Serializable {
    /*
     * static Hashtable primitiveToWrapper = new Hashtable(); static Hashtable wrapperToPrimitive = new Hashtable();
     * static { primitiveToWrapper.put( Boolean.TYPE, Boolean.class ); primitiveToWrapper.put( Byte.TYPE, Byte.class );
     * primitiveToWrapper.put( Short.TYPE, Short.class ); primitiveToWrapper.put( Character.TYPE, Character.class );
     * primitiveToWrapper.put( Integer.TYPE, Integer.class ); primitiveToWrapper.put( Long.TYPE, Long.class );
     * primitiveToWrapper.put( Float.TYPE, Float.class ); primitiveToWrapper.put( Double.TYPE, Double.class );
     * wrapperToPrimitive.put( Boolean.class, Boolean.TYPE ); wrapperToPrimitive.put( Byte.class, Byte.TYPE );
     * wrapperToPrimitive.put( Short.class, Short.TYPE ); wrapperToPrimitive.put( Character.class, Character.TYPE );
     * wrapperToPrimitive.put( Integer.class, Integer.TYPE ); wrapperToPrimitive.put( Long.class, Long.TYPE );
     * wrapperToPrimitive.put( Float.class, Float.TYPE ); wrapperToPrimitive.put( Double.class, Double.TYPE ); }
     */
    ...
    /**
     * VOID means "no type". Strictly speaking, this makes no sense here. But for practical reasons we'll consider the
     * lack of a type to be a special value.
     */
     ...
     *
     * @return the primitive TYPE class type of the value or Void.TYPE for Primitive.VOID or null value for type of
     * Primitive.NULL
     */
    ...
    /**
     * Perform a binary operation on two Primitives or wrapper types. If both original args were Primitives return a
     * Primitive result else it was mixed (wrapper/primitive) return the wrapper type. The exception is for boolean
     ...
     * @return corresponding "normal" Java type, "unwrapping" any bsh.Primitive types to their wrapper types.
     */
    ...
    /*
     * Unwrap Primitive wrappers to their java.lang wrapper values. e.g. Primitive(42) becomes Integer(42) @see #unwrap(
     * Object )
     */
    ...
    /**
     * Wrap primitive values (as indicated by type param) and nulls in the Primitive class. Values not primitive or null
     * are left unchanged. Primitive values are represented by their wrapped values in param value.
     * <p/>
    ...
    /**
     * Get the corresponding primitive TYPE class for the java.lang wrapper class type. e.g. Integer.class ->
     * Integer.TYPE
     */
    ...
    /**
     * Cast this bsh.Primitive value to a new bsh.Primitive value This is usually a numeric type cast. Other cases
     * include: A boolean can be cast to boolen null can be cast to any object type and remains null Attempting to cast
     * a void causes an exception
     *
    ...
    /*
     * Cast or check a cast of a primitive type to another type. Normally both types are primitive (e.g. numeric), but a
     * null value (no type) may be cast to any type. <p/>
     *
     * @param toType is the target type of the cast. It is normally a java primitive TYPE, but in the case of a null
     * cast can be any object type.
     *
     * @param fromType is the java primitive TYPE type of the primitive to be cast or null, to indicate that the
     * fromValue was null or void.
     *
     * @param fromValue is, optionally, the value to be converted. If checkOnly is true fromValue must be null. If
     * checkOnly is false, fromValue must be non-null (Primitive.NULL is of course valid).
     */
    ...
    /**
     * Cast a primitive value represented by its java.lang wrapper type to the specified java.lang wrapper type. e.g.
     * Byte(5) to Integer(5) or Integer(5) to Byte(5)
     *
