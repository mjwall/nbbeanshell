...
public class ClassManagerImpl extends BshClassManager {
    ...
    /**
     * The classpath of the base loader. Initially and upon reset() this is an empty instance of BshClassPath. It grows
     * as paths are added or is reset when the classpath is explicitly set. This could also be called the "extension"
     * class path, but is not strictly confined to added path (could be set arbitrarily by setClassPath())
     */
    private BshClassPath baseClassPath;
    private boolean superImport;
    /**
     * This is the full blown classpath including baseClassPath (extensions), user path, and java bootstrap path
     * (rt.jar)
     *
     * This is lazily constructed and further (and more importantly) lazily intialized in components because mapping the
     * full path could be expensive.
     *
     * The full class path is a composite of: baseClassPath (user extension) : userClassPath : bootClassPath in that
     * order.
     */
    ...
    /**
     * This handles extension / modification of the base classpath The loader to use where no mapping of reloaded
     * classes exists.
     *
    ...
    public URL getResource(String path) {
        URL url = null;
        if (baseLoader != null) // classloader wants no leading slash
        {
    ...
    // class reloading
    /**
     * Reloading classes means creating a new classloader and using it whenever we are asked for classes in the
     * appropriate space. For this we use a DiscreteFilesClassLoader
     */
     ...
     *
     * The special package name "<unpackaged>" can be used to refer to unpackaged classes.
     */
     ...
     *
     * public void reloadPathComponent( URL pc ) throws ClassPathException { throw new
    ...
    /*
     * Impl Notes: We add the bytecode source and the "reload" the class, which causes the BshClassLoader to be
     * initialized and create a DiscreteFilesClassLoader for the bytecode.
     *
     ...
     *
     * The listener list is implemented with weak references so that we will not keep every namespace in existence
     * forever.
     */
