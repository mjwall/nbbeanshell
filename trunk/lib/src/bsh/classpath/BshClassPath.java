...
/**
 * A BshClassPath encapsulates knowledge about a class path of URLs. It can maps all classes the path which may include:
 * jar/zip files and base dirs
 *
 * A BshClassPath may composite other BshClassPaths as components of its path and will reflect changes in those
 * components through its methods and listener interface.
 *
 * Classpath traversal is done lazily when a call is made to getClassesForPackage() or getClassSource() or can be done
 * explicitily through insureInitialized(). Feedback on mapping progress is provided through the MappingFeedback
 * interface.
 *
 * Design notes: Several times here we traverse ourselves and our component paths to produce a composite view of some
 * thing relating to the path. This would be an opportunity for a visitor pattern.
 */
public class BshClassPath
    ...
    /**
     * Explicitly set a class source. This is used for generated classes, but could potentially be used to allow a user
     * to override which version of a class from the classpath is located.
     */
    ...
    /**
     * If the claspath map is not initialized, do it now. If component maps are not do them as well...
     *
     * Random note: Should this be "insure" or "ensure". I know I've seen "ensure" used in the JDK source. Here's what
     * Webster has to say:
     *
     * Main Entry:ensure Pronunciation:in-'shur Function:transitive verb Inflected Form(s):ensured; ensuring : to make
     * sure, certain, or safe : GUARANTEE synonyms ENSURE, INSURE, ASSURE, SECURE mean to make a thing or person sure.
     * ENSURE, INSURE, and ASSURE are interchangeable in many contexts where they indicate the making certain or
     * inevitable of an outcome, but INSURE sometimes stresses the taking of necessary measures beforehand, and ASSURE
     * distinctively implies the removal of doubt and suspense from a person's mind. SECURE implies action taken to
    ...
    /**
     * @param topPath indicates that this is the top level classpath component and it should send the startClassMapping
     * message
     */
    ...
    /**
     * Get the full path including component paths. (component paths listed first, in order) Duplicate path components
     * are removed.
     */
    ...
    /**
     * Support for super import "*"; Get the full name associated with the unqualified name in this classpath. Returns
     * either the String name or an AmbiguousName object encapsulating the various names.
     */
    ...
    /*
     * Note: we could probably do away with the unqualified name table in favor of a second name source
     */
    ...
    /*
     * public void setNameCompletionIncludeUnqNames( boolean b ) { if ( nameCompletionIncludesUnqNames != b ) {
     * nameCompletionIncludesUnqNames = b; nameSpaceChanged(); } }
     */
    // Begin Static stuff
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            if (child.isDirectory()) {
                ...
                if (isClassFileName(name)) {
                    /*
                     * Remove absolute (topdir) portion of path and leave package-class part
                     */
    ...
    /**
     * Create a proper class name from a messy thing. Turn / or \ into ., remove leading class and trailing .class
     *
    ...
    public static URL[] getUserClassPathComponents()
        ...
        try {
            for (int i = 0; i < paths.length; i++) // We take care to get the canonical path first.
    ...
    public static class JarClassSource extends ClassSource {
        ...
        /*
         * Note: we should implement this for consistency, however our BshClassLoader can natively load from a JAR
         * because it is a URLClassLoader... so it may be better to allow it to do it.
         */
    ...
    void nameSpaceChanged() {
        ...
        for (int i = 0; i < nameSourceListeners.size(); i++) {
            ((NameSource.Listener) (nameSourceListeners.get(i))).nameSourceChanged(this);
        }
    }
    ...
    public static interface MappingFeedback {
         ...
         *
         * @param msg is a message about the path component being mapped @perc is an integer in the range 0-100
         * indicating percentage done public void classMapping( String msg, int perc );
         */
        /**
         * Provide feedback on the progress of mapping the classpath
         */
