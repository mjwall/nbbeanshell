...
/**
 * All of the reflection API code lies here. It is in the form of static utilities. Maybe this belongs in LHS.java or a
 * generic object wrapper class.
 */
/*
 * Note: This class is messy. The method and field resolution need to be rewritten. Various methods in here catch
 * NoSuchMethod or NoSuchField exceptions during their searches. These should be rewritten to avoid having to catch the
 * exceptions. Method lookups are now cached at a high level so they are less important, however the logic is messy.
 */
class Reflect {

    /**
     * Invoke method on arbitrary object instance. invocation may be static (through the object instance) or dynamic.
    ...
    /**
     * Invoke a method known to be static. No object instance is needed and there is no possibility of the method being
     * a bsh scripted method.
     */
     ...
     *
     * This method also deals with the field style property access. In the field does not exist we check for a property
     * setter.
     */
    ...
    /*
     * Note: this method and resolveExpectedJavaField should be rewritten to invert this logic so that no exceptions
     * need to be caught unecessarily. This is just a temporary impl. @return the field or null if not found
     */
    ...
    /**
     * Used when accessibility capability is available to locate an occurrance of the field in the most derived class or
     * superclass and set its accessibility flag. Note that this method is not needed in the simple non accessible case
     * because we don't have to hunt for fields. Note that classes may declare overlapping private fields, so the
     * distinction about the most derived is important. Java doesn't normally allow this kind of access (super won't
     * show private variables) so there is no real syntax for specifying which class scope to use...
     *
    ...
    /**
     * This method wraps resolveJavaMethod() and expects a non-null method result. If the method is not found it throws
     * a descriptive ReflectError.
     */
    ...
    /**
     * The full blown resolver method. All other method invocation methods delegate to this. The method may be static or
     * dynamic unless staticOnly is set (in which case object may be null). If staticOnly is set then only static
     * methods will be located.
     * <p/>
     *
		This method performs caching (caches discovered methods through the class manager and utilizes cached methods.)
     * <p/>
     *
	 	This method determines whether to attempt to use non-public methods based on Capabilities.haveAccessibility() and
     * will set the accessibilty flag on the method as necessary.
     * <p/>
     *
		If, when directed to find a static method, this method locates a more specific matching instance method it will
     * throw a descriptive exception analogous to the error that the Java compiler would produce. Note: as of 2.0.x this
     * is a problem because there is no way to work around this with a cast.
     * <p/>
     *
    ...
    /**
     * Climb the class and interface inheritence graph of the type and collect all methods matching the specified name
     * and criterion. If publicOnly is true then only public methods in *public* classes or interfaces will be returned.
     * In the normal (non-accessible) case this addresses the problem that arises when a package private class or
     * private inner class implements a public interface or derives from a public type.
     * <p/>
     *
    ...
    /**
     * Accumulate all methods, optionally including non-public methods, class and interface, in the inheritence tree of
     * baseClass.
     *
     * This method is analogous to Class getMethods() which returns all public methods in the inheritence tree.
     *
     * In the normal (non-accessible) case this also addresses the problem that arises when a package private class or
     * private inner class implements a public interface or derives from a public type. In other words, sometimes we'll
     * find public methods that we can't use directly and we have to find the same public method in a parent class or
     * interface.
     *
    ...
    /**
     * Primary object constructor This method is simpler than those that must resolve general method invocation because
     * constructors are not inherited.
     * <p/>
     * This method determines whether to attempt to use non-public constructors based on
     * Capabilities.haveAccessibility() and will set the accessibilty flag on the method as necessary.
     * <p/>
     */
    ...
    /*
     * This method should parallel findMostSpecificMethod() The only reason it can't be combined is that Method and
     * Constructor don't have a common interface for their signatures
     */
    ...
    /**
     * Find the best match for signature idealMatch. It is assumed that the methods array holds only valid candidates
     * (e.g. method name and number of args already matched). This method currently does not take into account Java 5
     * covariant return types... which I think will require that we find the most derived return type of otherwise
    ...
    /**
     * Implement JLS 15.11.2 Return the index of the most specific arguments match or -1 if no match is found. This
     * method is used by both methods and constructors (which unfortunately don't share a common interface for signature
     * info).
     *
     ...
     */
    /*
     * Note: Two methods which are equally specific should not be allowed by the Java compiler. In this case BeanShell
     * currently chooses the first one it finds. We could add a test for this case here (I believe) by adding another
     * isSignatureAssignable() in the other direction between the target and "best" match. If the assignment works both
     * ways then neither is more specific and they are ambiguous. I'll leave this test out for now because I'm not sure
     * how much another test would impact performance. Method selection is now cached at a high level, so a few friendly
    ...
    /**
     * Return a more human readable version of the type name. Specifically, array types are returned with postfix "[]"
     * dimensions. e.g. return "int []" for integer array instead of "class [I" as would be returned by Class getName()
     * in that case.
     */
    ...
    /**
     * A command may be implemented as a compiled Java class containing one or more static invoke() methods of the
     * correct signature. The invoke() methods must accept two additional leading arguments of the interpreter and
     * callstack, respectively. e.g. invoke(interpreter, callstack, ... ) This method adds the arguments and invokes the
     * static method, returning the result.
     */
