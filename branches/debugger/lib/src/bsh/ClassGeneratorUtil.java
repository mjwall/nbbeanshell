...
/**
 * ClassGeneratorUtil utilizes the ASM (www.objectweb.org) bytecode generator by Eric Bruneton in order to generate
 * class "stubs" for BeanShell at runtime. <p>
 *
 * Stub classes contain all of the fields of a BeanShell scripted class as well as two "callback" references to
 * BeanShell namespaces: one for static methods and one for instance methods. Methods of the class are delegators which
 * invoke corresponding methods on either the static or instance bsh object and then unpack and return the results. The
 * static namespace utilizes a static import to delegate variable access to the class' static fields. The instance
 * namespace utilizes a dynamic import (i.e. mixin) to delegate variable access to the class' instance variables. <p>
 *
 * Constructors for the class delegate to the static initInstance() method of ClassGeneratorUtil to initialize new
 * instances of the object. initInstance() invokes the instance intializer code (init vars and instance blocks) and then
 * delegates to the corresponding scripted constructor method in the instance namespace. Constructors contain special
 * switch logic which allows the BeanShell to control the calling of alternate constructors (this() or super()
 * references) at runtime. <p>
 *
 * Specially named superclass delegator methods are also generated in order to allow BeanShell to access overridden
 * methods of the superclass (which reflection does not normally allow). <p>
 *
 * @author Pat Niemeyer
 */
/*
 * Notes: It would not be hard to eliminate the use of org.objectweb.asm.Type from this class, making the distribution a
 * tiny bit smaller.
 */
public class ClassGeneratorUtil implements Constants {

    /**
     * The name of the static field holding the reference to the bsh static This (the callback namespace for static
     * methods)
     */
    static final String BSHSTATIC = "_bshStatic";
    /**
     * The name of the instance field holding the reference to the bsh instance This (the callback namespace for
     * instance methods)
     */
    ...
    /**
     * Generate a delegate method - static or instance. The generated code packs the method arguments into an object
     * array (wrapping primitive types in bsh.Primitive), invokes the static or instance namespace invokeMethod()
    ...
    void generateConstructor(
        ...
        // Generate code to push arguments as an object array
        generateParameterReifierCode(paramTypes, false/*
                 * isStatic
                 */, cv);
    ...
    /**
     * Generate a switch with a branch for each possible alternate constructor. This includes all superclass
     * constructors and all constructors of this class. The default branch of this switch is the default superclass
     * constructor. <p> This method also generates the code to call the static ClassGeneratorUtil getConstructorArgs()
     * method which inspects the scripted constructor to find the alternate constructor signature (if any) and evalute
     * the arguments at runtime. The getConstructorArgs() method returns the actual arguments as well as the index of
    ...
    /*
     * Generate a branch of the constructor switch. This method is called by generateConstructorSwitch. The code
     * generated by this method assumes that the argument array is on the stack.
     */
     ...
     * access to overridden methods of the superclass (which the Java reflection API normally does not allow).
     */
    ...
    /**
     * Generates the code to reify the arguments of the given method. For a method "int m (int i, String s)", this code
     * is the bytecode corresponding to the "new Object[] { new bsh.Primitive(i), s }" expression.
     *
    ...
    /**
     * Generates the code to unreify the result of the given method. For a method "int m (int i, String s)", this code
     * is the bytecode corresponding to the "((Integer)...).intValue()" expression.
     *
     * @param cv the code visitor to be used to generate the bytecode.
     * @author Eric Bruneton
     * @author Pat Niemeyer
    ...
    /**
     * Evaluate the arguments (if any) for the constructor specified by the constructor index. Return the
     * ConstructorArgs object which contains the actual arguments to the alternate constructor and also the index of
     * that constructor for the constructor switch.
     *
     * @param consArgs the arguments to the constructor. These are necessary in the evaluation of the alt constructor
     * args. e.g. Foo(a) { super(a); }
     * @return the ConstructorArgs object containing a constructor selector and evaluated arguments for the alternate
     * constructor
     */
    public static ConstructorArgs getConstructorArgs(
        ...
        try {
            constructors =
                    (DelayedEvalBshMethod[]) classStaticThis.getNameSpace().getVariable(BSHCONSTRUCTORS);
        } catch (Exception e) {
    ...
    /**
     * Initialize an instance of the class. This method is called from the **generated class** constructor to evaluate
     * the instance initializer and scripted constructor in the instance namespace.
     */
...
// TODO: clean up this conditional
        if (instanceThis == null) {
            ...
            // script backing it.
			/*
             * if ( classStaticThis == null ) { startInterpreterForClass( instance.getClass() ); // The class should now
             * be initialized with its ref, try again classStaticThis = getClassStaticThis( instance.getClass(),
             * className ); }
             */
...
//	System.out.println("constructor return type: "+constructor.getReturnType() );
     ...
     *
     * @return the class instance This object or null if the object has not been initialized.
     */
    ...
    /**
     * A ConstructorArgs object holds evaluated arguments for a constructor call as well as the index of a possible
     * alternate selector to invoke. This object is used by the constructor switch.
     *
    ...
    /**
     * Attempt to load a script named for the class: e.g. Foo.class Foo.bsh. The script is expected to (at minimum)
     * initialize the class body. That is, it should contain the scripted class definition.
     *
     * This method relies on the fact that the ClassGenerator generateClass() method will detect that the generated
     * class already exists and initialize it rather than recreating it.
     */
